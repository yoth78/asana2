  ...
  return (
    ...
    >
      ...
      <nav className="sidebar-nav" style={{ overflowY: 'auto', flex: 1, paddingBottom: '20px' }}>
        ...
        {(user.role === 'SUPER_ADMIN' || user.role === 'ADMIN') && !sidebarCollapsed && projects.length > 0 && (
          <div className="nav-section" style={{ marginTop: '20px' }}>
            <h4 className="section-title">PROJECTS</h4>
            <ul className="nav-list">
              {projects
                .filter(p => user.role === 'SUPER_ADMIN' || p.departmentId === user.departmentId)
                .map(project => (
                  <li key={project.id} className="nav-item" style={{ cursor: 'pointer' }} title={project.name} onClick={() => navigate(`/projects/${project.id}`)}>
                    <span className="project-color" style={{ backgroundColor: project.color, width: 8, height: 8, borderRadius: '50%', marginRight: 12, flexShrink: 0 }} />
