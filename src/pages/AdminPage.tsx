...
import {
  ...
  CheckSquare, Plus, Trash2, CheckCircle, XCircle, Bell
} from 'lucide-react';
...
const DepartmentsTab: React.FC = () => {
  const { departments, addDepartment, deleteDepartment } = useWorkspaceStore();
  const { allUsers } = useAuthStore();
        ...
        {departments.map(dept => (
          <div key={dept.id} className="card-panel col-span-4" style={{ borderTop: `4px solid ${dept.color}` }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              ...
              <button onClick={() => deleteDepartment(dept.id)} className="btn btn-ghost" style={{ padding: '0.5rem', color: 'var(--error)' }}>
                <Trash2 size={16} />
              </button>
            </div>
            <div style={{ marginTop: '1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '0.875rem', color: 'var(--text-secondary)' }}>
              <span>{dept.memberIds.length} members</span>
                ...
                <td style={{ padding: '1rem' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                    <select
                      value={user.role}
                      onChange={(e) => {
                        updateUserRole(user.id, e.target.value as Role, user.departmentId);
...
const InviteTab: React.FC<{ isSuperAdmin: boolean; fixedDepartmentId?: string }> = ({ isSuperAdmin, fixedDepartmentId }) => {
  const { inviteUser, invitations, acceptInvitation, declineInvitation } = useAuthStore();
  const { departments } = useWorkspaceStore();
  
  const [email, setEmail] = useState('');
  const [name, setName] = useState('');
  const [role, setRole] = useState<Role>('MEMBER');
  const [deptId, setDeptId] = useState(fixedDepartmentId || '');

  const handleInvite = (e: React.FormEvent) => {
    e.preventDefault();
    if (!email || !name || !deptId) {
      toast.error('Please fill all fields');
      return;
    }
    inviteUser(email, name, role, deptId);
    toast.success(`Invitation email sent to ${email}`);
    setEmail('');
    setName('');
  };

  const relevantInvitations = isSuperAdmin ? invitations : invitations.filter(i => i.departmentId === fixedDepartmentId);
    ...
    <div className="page-content bento-grid">
        ...
        <form onSubmit={handleInvite} className="card-panel">
          ...
          <div className="bento-grid">
            ...
            <div className="col-span-6" style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              ...
              {isSuperAdmin ? (
                <select required value={deptId} onChange={e => setDeptId(e.target.value)} className="select">
                  <option value="">Select Department</option>
                  {departments.map(d => <option key={d.id} value={d.id}>{d.name}</option>)}
                </select>
              ) : (
                <input disabled value={departments.find(d => d.id === fixedDepartmentId)?.name || ''} className="input" style={{ opacity: 0.7 }} />
              )}
            </div>
          </div>
          <div style={{ marginTop: '1.5rem' }}>
      ...
      <div className="card-panel col-span-12" style={{ padding: 0, overflow: 'hidden' }}>
        <div style={{ padding: '1.5rem', borderBottom: '1px solid var(--border-color)' }}>
          <h3 className="card-panel-title" style={{ margin: 0 }}>Pending Invitations</h3>
        </div>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
            <thead>
            ...
            <tbody>
              {relevantInvitations.map(inv => (
                <tr key={inv.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                  ...
                  <td style={{ padding: '1rem', color: 'var(--text-secondary)' }}>{departments.find(d => d.id === inv.departmentId)?.name}</td>
                  <td style={{ padding: '1rem' }}>
                  ...
                  <td style={{ padding: '1rem' }}>
                    {inv.status === 'pending' && (
                      <div style={{ display: 'flex', gap: '0.5rem' }}>
                        <button onClick={() => acceptInvitation(inv.id)} className="btn btn-ghost" style={{ padding: '0.5rem', color: 'var(--success)' }} title="Accept">
                          <CheckCircle size={18} />
                        </button>
                        <button onClick={() => declineInvitation(inv.id)} className="btn btn-ghost" style={{ padding: '0.5rem', color: 'var(--error)' }} title="Decline">
                          <XCircle size={18} />
                        </button>
                      </div>
                    )}
